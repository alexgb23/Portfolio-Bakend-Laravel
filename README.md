...
Backend principal y API REST del portfolio profesional, el laboratorio técnico público y el panel administrativo interno.

El proyecto está construido con Laravel bajo una arquitectura desacoplada (headless), pensado para servir contenido estructurado al frontend, exponer documentación técnica pública y mantener una base extensible para automatización, documentación e integraciones futuras.

🔗 Frontend: https://alex.syskovex.com/
...
Backend público: https://portfolio-api.syskovex.com/

Base URL API: https://portfolio-api.syskovex.com/api

Documentación UI: https://portfolio-api.syskovex.com/docs/api

OpenAPI JSON: https://portfolio-api.syskovex.com/docs/api.json

Panel admin: https://portfolio-api.syskovex.com/admin
...
En Render, los servicios gratuitos pueden entrar en reposo tras periodos de inactividad y generar cold starts.

Para reducir ese efecto, se utiliza un monitor HTTP externo que hace ping periódico al endpoint /health.

Este mecanismo mejora la disponibilidad percibida, aunque la forma oficialmente soportada de evitar el spin-down continuo sigue siendo usar un plan no gratuito.
...
Organización actual
La navegación del panel está agrupada por dominio funcional, principalmente Laboratorios y Proyectos.

Los recursos hijos de proyectos se gestionan desde relation managers dentro del recurso Projects, en lugar de exponerse como entradas separadas en el sidebar.

Esto mantiene una navegación más limpia y un flujo de edición más consistente dentro del panel.
...
Centralizar operaciones CRUD sin exponerlos públicamente.

🤖 Herramientas externas e integraciones
Google Apps Script
Google Apps Script se utiliza para gestionar respuestas automáticas de correo desde Gmail mediante el script responderCorreosDominio().

Acceso: https://script.google.com/home

Función principal: responderCorreosDominio()

Etiqueta de control: respondido-auto

Compatibilidad visual: los correos generados incluyen estilos para modo claro y oscuro mediante color-scheme y prefers-color-scheme.

Firma: utiliza una imagen remota transparente y adaptable desde syskovex.com.

No se incluyen claves, tokens ni URLs privadas de implementación en este README. Si el script se publica como aplicación web, guarda su URL de despliegue únicamente en variables seguras o en la configuración privada del proyecto.

Asistente experimental
Instancia pública del asistente conectada al ecosistema del laboratorio:

Asistente n8n / Hugging Face
